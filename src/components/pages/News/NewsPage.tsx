...
import { Edit, Delete, FilterList, Search, Event, Download } from "@mui/icons-material";
...
interface Article {
  ...
  date: string;
  tag: string;
  news_type: "OFFICIAL" | "EVENT" | "WARNING" | "MISCELLANEOUS";
  is_draft: boolean;
  archive: boolean;
  created_at: string;
}
...
const NEWS_TYPES = [
  ...
  { id: "EVENT", type_name: "Event", description: "Upcoming events" },
  { id: "WARNING", type_name: "Warning", description: "Important warnings" },
  {
    id: "MISCELLANEOUS",
...
export default function NewsPage() {
  ...
  const fetchNewsData = async () => {
    try {
      ...
      const formattedArticles = articlesResponse.data.data.map(
        (article: any) => ({
          ...article,
          status: article.archive
            ? "Archived"
            : article.is_draft
            ? "Draft"
            : "Published",
          ...
          tag:
            NEWS_TYPES.find((type) => type.id === article.news_type)
              ?.type_name || "Unknown",
        })
      );
  ...
  const handleExport = () => {
    ...
    const exportData = filteredArticles.map(article => ({
      ID: article.id,
      Title: article.title,
      Content: stripHtmlAndTruncate(article.content, 1000),
      Status: article.status,
      DateCreated: article.date,
      NewsType: article.tag,
      IsDraft: article.is_draft ? "Yes" : "No",
      IsArchived: article.archive ? "Yes" : "No",
      CreatedAt: new Date(article.created_at).toLocaleString()
    }));

    exportToExcel(exportData, 'news_articles_export');
  };
  ...
  const filteredArticles = articles
    ...
    .filter((article) => {
      ...
      if (statusFilter === "Published")
        return !article.is_draft && !article.archive;
      ...
      <Box className="news-container">
        ...
        <TableContainer component={Paper}>
          <Table className="news-table">
            <TableHead className="news-table-header">
              <TableRow>
                {[
                  "Title",
                  "Content Preview",
      ...
      <Box className="news-filters">
        <Box>
          ...
          >
            ...
            <MenuItem onClick={() => handleStatusFilter("Draft")}>
              Draft
            </MenuItem>
        ...
        <Box display="flex" gap={2}>
          <Button
            ...
            disabled={loading || !filteredArticles.length}
          >
            Export
          </Button>
          <Button
              ...
              filteredArticles
                ...
                .map((article) => (
                  <TableRow key={article.id}>
                    ...
                    <TableCell>
                      <Tooltip
                        title={stripHtmlAndTruncate(article.content, 200)}
                        arrow
                      >

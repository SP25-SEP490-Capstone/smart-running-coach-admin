...
interface DashboardData {
  ...
  total_exercise_sessions_today: number;
  total_admin_news: number;
  user_growth: {
    day: string;
    count: number;
...
export default function DashboardPage() {
  ...
  const growthPeriod = `${thirtyDaysAgo.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })} - ${today.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}`;
      ...
      {error ? (
        ...
        <>
          <Grid container spacing={3}>
            ...
            <Grid item xs={3}>
              <div className="stats-card">
                ...
                {loading ? (
                  ...
                  <div className="stats-card-value">{dashboardData?.total_experts}</div>
                )}
              </div>
            </Grid>
            <Grid item xs={3}>
            ...
            <Grid item xs={3}>
              <div className="stats-card">
                <div className="stats-card-title">Total Certificates</div>
                {loading ? (
                  <Skeleton width={100} height={30} />
                ) : (
                  <div className="stats-card-value">{dashboardData?.total_certificates}</div>
                )}
              </div>
            </Grid>
            <Grid item xs={3}>
              <div className="stats-card">
                <div className="stats-card-title">Total Schedules</div>
                {loading ? (
                  <Skeleton width={100} height={30} />
                ) : (
                  <div className="stats-card-value">{dashboardData?.total_schedules}</div>
                )}
              </div>
            </Grid>
            <Grid item xs={3}>
              <div className="stats-card">
                <div className="stats-card-title">Points Earned Today</div>
                {loading ? (
                  <Skeleton width={100} height={30} />
                ) : (
                  <div className="stats-card-value">{dashboardData?.total_points_today}</div>
                )}
              </div>
            </Grid>
            <Grid item xs={3}>
              <div className="stats-card">
                <div className="stats-card-title">Exercise Sessions Today</div>
                {loading ? (
                  <Skeleton width={100} height={30} />
                ) : (
                  <div className="stats-card-value">{dashboardData?.total_exercise_sessions_today}</div>
                )}
              </div>
            </Grid>
            <Grid item xs={3}>
              <div className="stats-card">
                <div className="stats-card-title">Total Admin News</div>
                {loading ? (
                  <Skeleton width={100} height={30} />
                ) : (
                  <div className="stats-card-value">{dashboardData?.total_admin_news}</div>
                )}
              </div>
            </Grid>
          </Grid>
          ...
          <Grid container spacing={3} mt={3} className='chart'>
            <Grid item xs={12}>
              <Card>
                <CardContent>
                  <p className="card-title">User Growth ({growthPeriod})</p>
                  {loading ? (
                    <Skeleton height={300} />
                  ) : error ? (
                    renderErrorState()
                  ...
                  dashboardData?.latest_posts.map(post => (
                    <div key={post.id} className="post-item" onClick={() => navigate(`/posts/${post.id}`)}>
                      ...
                      <div className="post-details">
                        <CommonAvatar uri={post?.user?.image?.url} size={20} />
                        <p>{post.User?.username} • {formatDate(post.created_at)}</p>
                      </div>
                    </div>
                  ))
                )}

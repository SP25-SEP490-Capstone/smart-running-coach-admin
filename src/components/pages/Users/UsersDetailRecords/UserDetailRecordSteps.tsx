...
import './UserDetailRecordSteps.scss';
...
interface UserDetailRecordStepsProps {
  userId: string;
}

export default function UserDetailRecordSteps({ userId }: UserDetailRecordStepsProps) {
  ...
  const fetchStepsData = async () => {
    ...
    try {
      ...
      const response = await aget<StepResponse>(
        `/record-steps/admin/${userId}?startTime=${startDate.toISOString()}&endTime=${endDate.toISOString()}`
      );
  ...
  const processChartData = (data: StepRecord[]) => {
    if (!data.length) {
      setChartData(null);
      return;
    }
    ...
    switch (timeRange) {
      case 'day':
        ...
        dataPoints = sortedData.map(record => record.count);
        break;
      case 'week':
        ...
        dataPoints = daysOfWeek.map(day => {
          ...
          return dayRecords.reduce((sum, record) => sum + record.count, 0);
        });
        break;
      case 'month':
      ...
      case 'year':
        ...
        dataPoints = monthsOfYear.map(month => {
          ...
          return monthRecords.reduce((sum, record) => sum + record.count, 0);
        });
        break;
    }
  ...
  const calculateSummary = (data: StepRecord[]) => {
    if (!data.length) {
      setSummary(null);
      return;
    }

    const steps = data.map(record => record.count);
  ...
  return (
    <Box className="user-detail-record-steps">
            ...
            <Paper elevation={2} className="summary-paper" sx={{ mt: 3 }}>
              <Grid container spacing={2}>
                <Grid item xs={6} md={3}>
                  ...
                  <Typography variant="h4">{summary.total.toLocaleString()}</Typography>
                </Grid>
                <Grid item xs={6} md={3}>
                ...
                <Grid item xs={6} md={3}>
                  ...
                  <Typography variant="h4">{summary.max.toLocaleString()}</Typography>
                </Grid>
                <Grid item xs={6} md={3}>
                  <Typography variant="subtitle2">Min</Typography>
                  <Typography variant="h4">{summary.min.toLocaleString()}</Typography>
                </Grid>
              </Grid>
            </Paper>
          )}
